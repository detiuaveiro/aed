...
static int _treeGetHeightREC(const struct _BSTreeNode* root) {
  ...
  int heightLeft = _treeGetHeightREC(root->left);
  int heightRight = _treeGetHeightREC(root->right);
...
void* BSTreeGetMin(const BSTree* header) {
  ...
  // ...
  //HIDE
  struct _BSTreeNode* n = header->root;
  while (n->left != NULL) {
    n = n->left;
  }
  return n->item;
...
static void _treeTraverseINOrder(struct _BSTreeNode* root,
  ...
  //SHOW function(root->item);
  //SHOW _treeTraverseINOrder(root->right, function);
  //SHOW _treeTraverseINOrder(root->left, function);
}
...
//HIDE
// Determina a altura da (sub)árvore que nasce no nó root.
// E acrescenta a *mismatches o número de nós que têm a altura errada.
// (Função RECURSIVA interna, que pode ser usada para verificação.)
static int _treeGetHeightREC2(const struct _BSTreeNode* root, int *mismatches) {
  // Complete a função
  // ...
  if (root == NULL) return -1;
  int heightLeft = _treeGetHeightREC2(root->left, mismatches);
  int heightRight = _treeGetHeightREC2(root->right, mismatches);
  int height = 1 + ((heightLeft > heightRight) ? heightLeft : heightRight);
  if (height != root->height) (*mismatches)++;   // increase number of mismatches
  return height;
}
//SHOW
